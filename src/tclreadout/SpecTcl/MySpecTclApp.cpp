...
// For event built data we need the event built data unpacker
// and unpackers for each module:
...
{
  /// Note: Lifetimes of objects created here are the lifetime of the program
  //        so there are not really any leaks here
  
  // For the CAEN modules we need to register the
  // event build unpacker and then an unpacker for each module
  // The top level processor will make diagnostic parameters that
  // will show e.g. time differences between the modules
  // within built events so clock difference drifts can be monitored.

  auto pTopLevel = new
    caen_spectcl::VX2750EventBuiltEventProcessor("diagnostic");

  // For each module/data source we need to register an unpacker
  // for the raw data from that module:
  // The parameters for constructionare the module name (from the config file)
  // and the base name for parameters created by the processor.
  //
  pTopLevel->addEventProcessor(
      1, *(new caen_spectcl::VX2750ModuleUnpacker("adc1", "adc1-params"))
  );
  
  // Register the top level event processor so it is called for each event
  // Additional event processors can be registered to find and create additional
  // parameter from the raw processor.
  //
